...
/*		if(memDto.getMemGroupMap().get(subjectVo.getGroupNum()) == 'T') {
	...
	@RequestMapping(value="/board/delete/subject", method = RequestMethod.POST)
	public String delSubject(MemDto memDto, SubjectVo subjectVo) {
		subjectService.deleteSubject(subjectVo.getSubjectNum());
		return "forward:/group/setting-subject";
	}
